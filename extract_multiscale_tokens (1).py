...
VAE_MODEL_PATH = "weights/infinity_vae_d56_f8_14_patchify.pth"  # VAE模型路径（匹配您的配置）
OUTPUT_DIR = "output/extracted_tokens"  # 输出目录

# VAE配置参数（完全匹配您的inf.py配置）
VAE_CONFIG = {
    'vae_type': 14,  # codebook_dim=14（匹配您的配置）
    'apply_spatial_patchify': 1,  # 1表示使用spatial patchify（匹配您的配置）
    'schedule_mode': "dynamic",
    'patch_size': 8,  # 使用8x8 patch（匹配f8_14_patchify）
    'encoder_ch_mult': [1, 2, 4, 4],  # 4层编码器（spatial patchify配置）
    'decoder_ch_mult': [1, 2, 4, 4],  # 4层解码器（spatial patchify配置）
}
...
def main():
    ...
    extraction_result = extractor.extract_multiscale_tokens(
        image_path=IMAGE_PATH,
        h_div_w=1.0,  # 1:1宽高比
        pn='0.25M'    # 0.25M像素级别，对应512x512分辨率（匹配您的配置）
    )
